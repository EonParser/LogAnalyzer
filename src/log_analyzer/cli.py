...
from .processors.pipeline import Pipeline, FilterStep
...
def analyze(
    ...
    for expr in filter:
        pipeline.add_step(
            FilterStep(f'filter_{expr}', eval(f'lambda e: {expr}'))
        )
    ...
    with Progress(
        ...
        for file_path in files:
            ...
            task = progress.add_task(
                ...
                total=estimate_line_count(path)
            )
...
def list_parsers():
    ...
    for name, parser_class in analyzer.parser_factory._parsers.items():
        table.add_row(
            name,
            parser_class.__doc__ or "No description"
        )
