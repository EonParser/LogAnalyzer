    ...
    def _parse_rfc5424(self, match: re.Match, line: str) -> LogEntry:
        ...
        try:
            ...
            if data["structured_data"] and data["structured_data"] != "-":
                for block in re.finditer(r"\[([^\]]+)\]", data["structured_data"]):
            ...
            return LogEntry(
                ...
                metadata={"log_type": "syslog", "format": "rfc5424"},
            )

        except (ValueError, KeyError) as e:
            raise ParserError(f"Error parsing RFC 5424 syslog: {str(e)}") from e
    ...
    def _parse_bsd(self, match: re.Match, line: str) -> LogEntry:
        ...
        try:
            ...
            timestamp = datetime.strptime(
                f"{data['timestamp']} {current_year}", "%b %d %H:%M:%S %Y"
            )
            ...
            return LogEntry(
                ...
                raw_data=line,
                parsed_data=parsed_data,
                metadata={"log_type": "syslog", "format": "bsd"},
            )

        except (ValueError, KeyError) as e:
            raise ParserError(f"Error parsing BSD syslog: {str(e)}") from e
...
class SystemdJournalParser(BaseParser):
    ...
    def supports_format(self, line: str) -> bool:
        ...
        try:
            ...
            return all(
                field in data
                for field in ["__REALTIME_TIMESTAMP", "MESSAGE", "PRIORITY"]
            )
    ...
    def parse_line(self, line: str) -> Optional[LogEntry]:
        ...
        try:
            ...
            severity = SyslogParser.SEVERITY_MAP.get(int(data["PRIORITY"]), "INFO")
            ...
            return LogEntry(
                ...
                raw_data=line,
                parsed_data=parsed_data,
                metadata={"log_type": "journal", "format": "systemd"},
            )

        except (json.JSONDecodeError, KeyError, ValueError) as e:
            raise ParserError(f"Error parsing systemd journal: {str(e)}") from e
