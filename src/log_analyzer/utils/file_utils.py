...
def calculate_checksum(
    path: Path, algorithm: str = "sha256", chunk_size: int = 8192
) -> str:
...
@contextmanager
def atomic_write(
    path: Union[str, Path], mode: str = "w", encoding: Optional[str] = None, **kwargs
) -> Iterator[TextIO]:
...
def tail(path: Union[str, Path], lines: int = 10, chunk_size: int = 8192) -> List[str]:
