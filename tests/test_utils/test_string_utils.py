    ...
    def test_extract_between(self):
        ...
        for text, start, end, expected in test_cases:
            ...
            if expected:
                ...
                assert all(
                    s in m and e in m
                    for m, (s, e) in zip(with_markers, zip(expected, expected))
                )
...
class TestTextAnalysis:
    ...
    def test_find_patterns(self):
        ...
        test_cases = [
            # Repeated phrases
            ("the cat and the dog and the cat", ["the cat", "and the"]),
            # Multiple patterns
            ("red blue red green blue red", ["red blue", "blue red"]),
            # Minimum length
            ("a a a b b b", []),
            # No patterns
...
class TestJSONOperations:
    ...
    def test_contains_json(self):
        ...
        test_cases = [
            # Valid JSON
            ('Before {"key": "value"} After', (7, 23)),
            # Nested JSON
            ('{"outer": {"inner": "value"}}', (0, 31)),
            # Invalid JSON
            ('{"invalid": missing quotes}', None),
            # Multiple JSON objects (should find first)
            ('{"first": 1} {"second": 2}', (0, 12)),
            # Edge cases
            ("", None),
            ("{}", (0, 2)),
        ]
