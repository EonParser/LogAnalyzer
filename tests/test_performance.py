...
class TestMemoryUsage:
    ...
    def test_memory_usage_large_file(self, analyzer, large_log_file):
        ...
        print(
            f"Memory per entry: {memory_increase / results['total_entries'] / 1024:.2f} KB"
        )
    ...
    def test_multi_worker_performance(self, analyzer, tmp_path, benchmark):
        ...
        def process_with_workers(workers):
            return analyzer.analyze_directory(
                tmp_path, pattern="*.log", max_workers=workers
            )
        ...
        for workers in worker_counts:
            results = benchmark(process_with_workers, workers, rounds=3)

            total_entries = results["total_entries"]
            print(f"\nWorkers: {workers}")
            print(f"Total entries: {total_entries}")
            print(
                f"Processing speed: {results['entries_per_second']:.2f} entries/second"
            )
...
class TestPipelinePerformance:
    ...
    def test_pipeline_complexity(self, analyzer, large_log_file, benchmark):
        ...
        pipelines = {"simple": Pipeline(), "medium": Pipeline(), "complex": Pipeline()}

        # Simple pipeline: just one transformation
        pipelines["simple"].add_step(
            TransformStep("simple", TransformerFactory.create_standard_transformer())
        )

        # Medium pipeline: multiple transformations
        medium_transformer = TransformerFactory.create_standard_transformer()
        pipelines["medium"].add_step(TransformStep("medium1", medium_transformer))
        pipelines["medium"].add_step(
            TransformStep("medium2", TransformerFactory.create_security_transformer())
        )
        ...
        pipelines["complex"].add_step(TransformStep("complex1", complex_transformer))
        pipelines["complex"].add_step(
            TransformStep("complex2", TransformerFactory.create_security_transformer())
        )
        ...
        for name, pipeline in pipelines.items():
            ...
            def process_with_pipeline():
                return analyzer.analyze_file(
                    large_log_file, parser_name="apache", pipeline=pipeline
                )

            results = benchmark(process_with_pipeline, rounds=3)
