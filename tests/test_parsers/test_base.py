...
from log_analyzer.parsers.base import (
    BaseParser,
    LogEntry,
    ParserError,
    ParserFactory
...
def test_parser_factory_detection():
    ...
    factory.register_parser(
        "parser1",
        lambda: SimpleParser(format_pattern="format1")
    )
    factory.register_parser(
        "parser2",
        lambda: SimpleParser(format_pattern="format2")
    )
...
def test_log_entry_validation():
    ...
    with pytest.raises(TypeError):
        LogEntry(
            timestamp=timestamp,
            level="INFO"  # Missing required fields
        )
...
def test_parser_factory_thread_safety():
    ...
    threads = [
        threading.Thread(target=register_parser)
        for _ in range(10)
    ]
