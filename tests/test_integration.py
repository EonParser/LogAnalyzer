...
def test_compressed_file_handling(analyzer, sample_logs):
    ...
    results = analyzer.analyze_file(
        sample_logs / "apache_access.log.gz",
        parser_name='apache'
    )
...
def test_error_detection_pipeline(analyzer, sample_logs):
    ...
    pipeline.add_step(
        FilterStep(
            'error_filter',
            lambda e: e.level in {'ERROR', 'CRITICAL'} or 
    ...
    pipeline.add_step(
        TransformStep('enrich', transformer)
    )
    ...
    assert results['total_entries'] > 0
    assert all(
        entry.level in {'ERROR', 'CRITICAL'} or 
        entry.parsed_data.get('status', 200) >= 400
...
def test_security_analysis_pipeline(analyzer, sample_logs):
    ...
    pipeline.add_step(
        TransformStep('security_transform', transformer)
    )
    ...
    pipeline.add_step(
        FilterStep(
            'suspicious_ips',
            lambda e: (
                e.parsed_data.get('status', 200) >= 400 or
                e.level in {'ERROR', 'CRITICAL'}
            )
        )
    )
...
def test_concurrent_processing(analyzer, sample_logs):
    ...
    for i in range(5):
        ...
        with open(log_file, 'w') as f:
            for j in range(1000):
                f.write(f'192.168.1.100 - - [10/Feb/2024:13:55:{j:02d} +0000] "GET /api/test HTTP/1.1" 200 100 "-" "-"\n')

    results = analyzer.analyze_directory(
        sample_logs,
        pattern="large_file_*.log",
        max_workers=3
    )
